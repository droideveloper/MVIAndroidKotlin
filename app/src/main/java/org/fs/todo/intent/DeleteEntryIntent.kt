...
class DeleteEntryIntent(private val entry: Entry, private val entryRepository: EntryRepository): ObservableIntent<EntryModel>() {
  ...
  private fun bySuccess(entry: Entry): Observable<Reducer<EntryModel>> = Observable.just(
      { model -> model.copy(state = Operation(C.DELETE), data = listOf(entry)) },
